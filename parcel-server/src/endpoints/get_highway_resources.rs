...
pub async fn get_highway_resources(
    ...
    if let Some(constructions) = &request.constructions {
        ...
        let likes = conn
            .likes()
            .get_total_highway_likes(vec![session.account_id.as_str()])
            .await?;

        Ok(Json(GetHighwayResourcesResponse {
            construction_contributors: Vec::new(),
            put_resources: Vec::new(),
            users_like: *likes.get(&session.account_id).unwrap_or(&0),
        }))
    }
}
